...
get host
socket to to other side
connect
send what you received to server
receive
send back to browser
	//GET HOST OF SERVER PROJ 2
	struct hostent *server2;
	struct sockaddr_in si_server2;
	struct	hostent *hostServer2 = gethostbyname(HOSTFROMURL);
	if(hostServer2 == NULL) {
		printf("hostname error\n");
		exit;
	}
...
printf("recieve/n");
	while (1) {
		...
		bzero(request, 1000);
		printf("\nrequest: %s\n", request);
		recv(tcpsocket2, request, 999, 0)
		if (request[]=='</html>'){
			//close file and send to browser
		}
	}
//EXTRA CODE
bzero(request, 1000);
